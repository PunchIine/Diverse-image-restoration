...
class pdgan(BaseModel):
    ...
    def modify_options(parser, pd_is_train=True):
        ...
        if pd_is_train:
            ...
            parser.add_argument('--lambda_p', type=float, default=3.0, help='weight')
    ...
    def test(self, img_p, num):
        ...
        for i in range(self.opt.nsampling):
            ...
            self.img_g, _ = self.net_G_pd(z, self.mask, img_p)
            self.img_out = (1 - self.mask) * self.img_g[-1].detach() + self.mask * self.img_m
            self.score = self.net_D_pd(self.img_out, self.mask)
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        D_real_loss = (D_real_loss1 + D_real_loss2 + D_real_loss3 * 2) / 3.0
        # fake
        # print("fake" + str(fake.shape))
        D_fake = netD(fake.detach(), self.mask)
        ...
        D_fake_loss = (D_fake_loss1 + D_fake_loss2 + D_fake_loss3 * 2) / 3.0
    ...
    def backward_G(self):
        ...
        # g loss fake
        D_fake_A = self.net_D_pd(self.img_g_A[-1], self.mask)
        D_fake_B = self.net_D_pd(self.img_g_B[-1], self.mask)
        ...
        self.loss_ad_g_A = (self.loss_ad_g_A1 + self.loss_ad_g_A2 + self.loss_ad_g_A3 * 2) / 3.0
        ...
        self.loss_ad_g_B = (self.loss_ad_g_B1 + self.loss_ad_g_B2 + self.loss_ad_g_B3 * 2) / 3.0
        ...
        for i, (img_fake_i, img_real_i, mask_i) in enumerate(zip(self.img_g_A, self.scale_img, self.scale_mask)):
            loss_app_g_A += self.L1loss(img_fake_i, img_real_i)
        ...
        for i, (img_fake_i, img_real_i, mask_i) in enumerate(zip(self.img_g_B, self.scale_img, self.scale_mask)):
            loss_app_g_B += self.L1loss(img_fake_i, img_real_i)
        ...
        # diversity loss
        self.loss_diver = 0.5 * (1 / (self.Diversity_loss(self.img_g_A[-1], self.img_g_B[-1]) + 1e-5))
